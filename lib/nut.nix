...
let
  ...
  runTests =
    ...
    (
      let
        ...
        results = map (test: addFilePath filepath (evaluateTest test)) testsToRun;

        failed = builtins.filter (test: !test.passed) results;
        passed = builtins.filter (test: test.passed) results;
      in
      {
  ...
  evaluateTest =
    ...
    if test.actual == test.expected then
      {
        inherit (test) name actual expected;
        passed = true;
      }
    else
      {
