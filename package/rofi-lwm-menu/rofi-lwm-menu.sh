...
# Derived from code in rofi-power-menu:
# See https://github.com/jluttine/rofi-power-menu/COPYING for more information
...
declare -A actions
actions[reload]="##pkgs.qtile##/bin/qtile cmd-obj -o cmd -f reload_config"
# shellcheck disable=SC2089
actions[quit]="loginctl terminate-user \'\'"
...
while true; do
    case "$1" in
        "-h"|"--help")
            echo "rofi-lwm-menu - a power menu mode for Rofi"
            echo
            echo "Usage: rofi-lwm-menu [--choices CHOICES] [--confirm CHOICES]"
            ...
            echo
            echo "  rofi -show menu -modi \"menu:rofi-lwm-menu --choices=shutdown/reboot\""
            echo
...
if [ -z "${selection+x}" ]
then
    echo -e "\0prompt\x1fLooniversity Window Manager"
    for entry in "${show[@]}"
    do
